<script lang="ts">
	...
	let { people, hidden, sheet = $bindable(), ontoggle, onprint }: Props = $props();
	...
	{#each people as person (person.id)}
		...
		>
			<span
				...
				class:border-2={person.background}
				class:opacity-30={off}
				style:background={person.background ? 'transparent' : person.color}
